...
static inline uint64_t WQdequeue(volatile struct work_queue *queue){
    ...
    volatile uint64_t *address =& queue->data[*head];
...
static inline int serve_request(struct transfer_t *ts, uint64_t pid)
{
    ...
    uint64_t off = 0;
    // uint64_t pid, vpid;
    volatile struct transfer_t *vts=ts;
    ...
    for(int i = 0; i < MAX_PROCESS; i++){
        if (vts->pid_info[i].pid == 0)
            break;
        // pr_err("pid:%ld, pid_info[i]:%ld\n", pid, ts->pid_info[i].pid);
        if (vts->pid_info[i].pid == pid)
            if (vts->pid_info[i].done){
                return 0;
            }
    }

    for( i = 0; i < vts->nr_pi; i++) {
        // TODO: send request to parasite
        if ( vts->page_info[i].pid == pid ){
            #ifdef PID_DEBUG
            pr_err("TSpie serve dst:%lx, src:%lx, len:%lx\n",((uint64_t)get_mem(vts) + off * 4096),vts->page_info[i].addr,vts->page_info[i].leng * 4096);
            pr_err("access mem:%lx\n",*(uint64_t *)(vts->page_info[i].addr+vts->page_info[i].leng * 4096-1024));
            #endif
            memcpy((void *)((uint64_t)get_mem(vts) + off * 4096), (void *)vts->page_info[i].addr, vts->page_info[i].leng * 4096);
            #ifdef PID_DEBUG
            pr_err("memcpy done\n");
            #endif
        ...
        if ( i == vts->nr_pi - 1){
            ...
            for(int j = 0; j < MAX_PROCESS; j++){
                if (vts->pid_info[j].pid == 0)
                    break;
                if (vts->pid_info[j].pid == pid)
                    vts->pid_info[j].done = 1;
                if (vts->pid_info[j].done == 0)
                    done = 0;
            }
            if (done){
                pr_err("done\n");

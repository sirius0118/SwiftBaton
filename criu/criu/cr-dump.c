...
static int dump_one_task(struct pstree_item *item, InventoryEntry *parent_ie)
{
	...
	pr_warn("run to hereindex:%d\n", index);
	i = 0;
	list_for_each_entry(vma_i, &vmas.h, list)
		i++;
	pr_warn("run to herei:%d\n", i);
	...
	list_for_each_entry(vma_i, &vmas.h, list) {
		...
		memset(PidVma[index]->vmas[i].bitmap, 0, round_up((vma_i->e->end - vma_i->e->start) / PAGE_SIZE, 64) / 8);
		i++;
	}
...
#else
	thread_pstree_switch_state(root_item, (ret || post_dump_ret) ? TASK_ALIVE : opts.final_state);
#endif
	timing_stop(TIME_FROZEN);
	free_pstree(root_item);
	seccomp_free_entries();
...
int cr_dump_tasks(pid_t pid)
{
	...
	ret = write(sync_pretransfer, &item_num, 4);
	ret = write(sync_pretransfer, pre_mr, sizeof(struct data_buffer) * item_num);
	...
	pr_warn("run to here item_num:%d\n", item_num);
	...
	*(uint64_t *)mem = item_num;
	...
	// FT_res.buf = (char *)PrefetchRegions;
	FT_res.mr = (struct ibv_mr **)calloc(1, sizeof(struct ibv_mr *));

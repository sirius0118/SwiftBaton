...
#ifdef PARALLEL_DUMP
	// step1: enter into the namespace of target process
	if (switch_ns(pid, &net_ns_desc, &nsret))
		return NULL;
	// step2: create unix socket
	ret = unix_sock = socket(PF_UNIX, SOCK_SEQPACKET | SOCK_NONBLOCK, 0);
	if (ret < 0)
		pr_err("Can't create socket: %d\n", ret);
	// step3: restore the namespace
	if (nsret >= 0 && restore_ns(nsret, &net_ns_desc)){
		pr_err("Can't restore namespace\n");
		return NULL;
	}
	// step4: set the socket to the parasite_ctl
	ictx->sock = unix_sock;
#endif

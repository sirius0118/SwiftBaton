...
int read_pstree_ids(struct pstree_item *pi)
{
	int ret;
	struct cr_img *img;
...
// 		pi->threads[i].item = NULL;
// 		if(i == 0)
// 			continue; // A thread leader [pid] is in a tree already
// 		node = lookup_create_pid(pi->threads[i].ns[0].virt, &pi->threas[i]);
// 		BUG_ON(node == NULL);
// 		if(node != &pid->threads[i]){
// 			pr_err("Unexpected task %d in a tree %d\n", e->threads[i], i);
// 			goto err;
// 		}
// 	}
...
// 	// --------------- read others -----------------
// 	// if (kpid_max && pid_max > kpid_max){
// 	// 	// Try to set kernel pid max
...
// int prepare_pstree_noroot(void)
// {
// 	int ret;
// 	pid_t pid_max = 0, kpid = 0, pid;
// 	int fd;
// 	char buf[21];
// 	struct cr_img *img;
...
// 		if (e->pid == root_item->pid->ns[0].virt)
// 			continue;

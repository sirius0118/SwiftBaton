...
int RDMA_parasite_dump_pages_seized(struct pstree_item *item, struct vm_area_list *vma_area_list, struct mem_dump_ctl *mdc, struct parasite_ctl *ctl)
{
	...
	if (ret) {
		pr_err("Can't create pagefault shregion with parasite\n");
		return ret;
	}
	ret = compel_rpc_sync(PARASITE_CMD_CREATE_PAGEFAULT_SHREGION, ctl);
	if (ret) {
		pr_err("Can't receive the pagefault shared memory\n");
		return ret;
	}
	...
	if (item == root_item){
		...
		*(uint64_t *)(tsmem_args) = item->pid->ns[0].virt;
	...
	if (item == root_item){
		...
		pr_warn("得到的TS地址为:%lx, 大小为:%lx\n", tsmem_addr, tsmem_size);

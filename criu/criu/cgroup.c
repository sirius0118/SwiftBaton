...
int dump_root_cgroup(struct pstree_item *item, char *contents, u32 *cg_id)
{
	int pid, tid;
	LIST_HEAD(ctls);
	unsigned int n_ctls = 0;
	struct cg_set *cs;
	struct dump_cgroup_args{
...
pr_warn("run to here\n");
	...
	tid = pid;
pr_warn("run to here\n");
	args = (struct dump_cgroup_args *)malloc(sizeof(*args));
	memcpy(args->contents, contents, 4096 - 32);
	// args->contents = contents;
pr_warn("run to here, contents:%s\n", contents);
	if (parse_thread_cgroup(pid, tid, (struct parasite_dump_cgroup_args *)args, &ctls, &n_ctls))
		return -1;
...
int dump_thread_cgroup(const struct pstree_item *item, u32 *cg_id, struct parasite_dump_cgroup_args *args, int id)
{
	...
	if (enter_multi_process){
		if(item != root_item)
			futex_wait_while(&barriers->cg_root_item, 0);
	...
	if (enter_multi_process){
		...
		futex_set_and_wake(&barriers->cg_root_item, 1);
	}
#endif
	*cg_id = cs->id;
	return 0;

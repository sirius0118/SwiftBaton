...
struct futex_barriers{
	...
	mutex_t new_sock;
	futex_t cg_root_item, num_process;
...
#ifdef PARALLEL_DUMP
	compel_run_id++;
	mutex_unlock(&barriers->new_sock);
#endif
...
static int prepare_tsock(struct parasite_ctl *ctl, pid_t pid, struct parasite_init_args *args)
{
	...
	ssock = ctl->ictx.sock;
	// ssock = socket(PF_UNIX, SOCK_SEQPACKET | SOCK_NONBLOCK, 0);
	// if(ssock < 0){
	// 	pr_err("Can not create unix socket\n");
	// 	goto err;
	// }

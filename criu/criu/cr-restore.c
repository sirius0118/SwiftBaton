...
void barrier_init(void){
	futex_init(&restore_barriers.NS_create);
}
#endif
...
static inline int fork_with_pid(struct pstree_item *item)
{
    ...
    //     exit(1);
    // }
	// ret = open(PIPE_NAME, O_WRONLY | O_NONBLOCK);
	// ret = write(ret, &root_item->pid->real, 4);
...
static int __restore_task_with_children(void *_arg)
{
	...
	if (current->parent == NULL) {
		...
		if (root_ns_mask & CLONE_NEWNET) {
			...
			if (ns->ext_key)
				...
				ret = get_service_fd(CRIU_NS_PID);
				sprintf(nspath, "/proc/%d/ns/net", ret);
				nsfd = open(nspath, O_RDONLY);
				ret = switch_ns_by_fd(nsfd, &net_ns_desc, NULL);
			}
			if (ret) {
				pr_perror("Can't switch to target net-namespace");
				goto err;
			}
		}
#else
    ...
    pr_info("Join the mnt namespace\n");
...
int run_page_client(void)
{
	...
	pid = fork();
	if (pid == 0){
		sprintf(cmd, "sudo criu lazy-pages -D %s -W %s --page-server --address %s --port %d", opts.imgs_dir, opts.work_dir, opts.addr, opts.port);
		ret = system(cmd);
		exit(0);
	}
	return ret;
...
int do_mnt_precreate(void){
	int ret;

	pr_warn("opts.work_dir=%s\n", opts.work_dir);
	// Create a fake root_item
	root_item = __alloc_pstree_item(true);
	root_item->pid->ns[0].virt = 1;
	root_item->pid->real = getpid();
	// ret = read_pstree_ids(root_item);
	root_item->ids = (TaskKobjIdsEntry *)malloc(sizeof(TaskKobjIdsEntry));

	root_item->ids->has_mnt_ns_id = 1;

	root_item->ids->vm_id = 2;
	root_item->ids->files_id = 2;
	root_item->ids->fs_id = 2;
	root_item->ids->sighand_id = 2;
	root_item->ids->pid_ns_id = 9;
	root_item->ids->net_ns_id = 10;
	root_item->ids->ipc_ns_id = 11;
	root_item->ids->uts_ns_id = 12;
	root_item->ids->mnt_ns_id = 13;

	root_item->ids->user_ns_id = 7;
	root_item->ids->cgroup_ns_id = 8;
	root_item->ids->time_ns_id = 5;

	root_item->parent = NULL;
	root_item->children.prev = root_item->children.next = &root_item->children;

	if (root_item->ids->has_mnt_ns_id) {
		if (rst_add_ns_id(root_item->ids->mnt_ns_id, root_item, &mnt_ns_desc))
			return -1;
	}

	// Restore ids for this fake root_item
	ret = prepare_pstree_kobj_ids();

	// Create mnt namespace
	if (mntns_maybe_create_roots()){
		pr_err("mntns_maybe_create_roots failed\n");
		return -1;
	}
	pr_debug("mntns_maybe_create_roots success\n");

	if (read_mnt_ns_img()){
		pr_err("read_mnt_ns_img failed\n");
		return -1;
	}
	pr_debug("read_mnt_ns_img success\n");

	if (prepare_mnt_ns()){
		pr_err("prepare_mnt_ns failed\n");
		return -1;
	}
	pr_debug("do_mnt_precreate success\n");
	root_item =NULL;
	ret = chdir(opts.work_dir);
...
int placehold(void){
	...
	// sprintf(nspath, "/proc/%d/ns/net", nspid);
	// nsfd = open(nspath, O_RDONLY);
	ret = install_service_fd(CRIU_NS_PID, nspid);
	return nspid;
}
...
int cr_restore_tasks(void)
{
	...
	// temp
	// char sync_addr[50] = "10.0.0.62";
	// int sync_port = 4567;

	// opts.sync_addr = sync_addr;
	// opts.sync_port = sync_port;

	// opts.addr = (char *)malloc(20);
	// sprintf(opts.addr, "10.0.0.62");
	// opts.port = 12345;
	...
	if (ret < 0)
		goto err;
	nspid = placehold();
	pi = (struct pstree_item *)malloc(sizeof(struct pstree_item));
	pi->pid = (struct pid *)malloc(sizeof(struct pid));
	root_item = pi;
	root_item->pid->real = nspid;
	ret = run_scripts(ACT_SETUP_NS);
	free(root_item);
	root_item = NULL;
	if (ret)
	...
	fputs("stop", fp);
	fclose(fp);
	update_state(sync_fd, START_PROCESS_DUMP);
	wait_state(sync_fd, END_PROCESS_DUMP);
	
	update_state(page_sync, END_PROCESS_DUMP);
...
clean_cgroup:
	fini_cgroup();
err:
	nspid = get_service_fd(CRIU_NS_PID);
	ret = kill(nspid, SIGTERM);
	fp = fopen("ready", "w");
	fputs("stop", fp);
	fclose(fp);
...
#ifdef MUL_UFFD
	// for (i = 0; i < task_args->uffd_set.uffd_region_n; i++)
	// 	RST_MEM_FIXUP_PPTR(task_args->uffd_set.uffd_region[i].vma);
	// RST_MEM_FIXUP_PPTR(task_args->uffd_set.uffd_region);
#endif
...
#endif
pr_warn("run to herecurrent->nr_threads:%d, i:%d\n", current->nr_threads, i);
		...
		gpregs = *CORE_THREAD_ARCH_INFO(tcore)->gpregs;
		thread_args[i].gpregs = gpregs;
